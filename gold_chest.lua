	...
	on_dig = function(pos, node, digger)
		...
		if digger:get_wielded_item():get_tool_capabilities().groupcaps.locked then
			if digger:get_wielded_item():get_tool_capabilities().groupcaps.locked.maxlevel >= 3 then
				...
				if math.random() > math.pow(.66, wieldlevel - 4) then --66% to the power of level difference - silver breaks
					minetest.set_node(pos, {name="technic:gold_chest",paramtype2="facedir"})
